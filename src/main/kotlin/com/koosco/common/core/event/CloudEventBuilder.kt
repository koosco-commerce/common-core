...
class CloudEventBuilder<T> private constructor() {
    ...
    companion object {
         ...
         */
        fun <T> builder(source: String, type: String): CloudEventBuilder<T> =
            CloudEventBuilder<T>()
                .source(source)
                .type(type)
    }
 ...
 * ```
 */
fun <T> cloudEvent(init: CloudEventBuilder<T>.() -> Unit): CloudEvent<T> {
    return CloudEventBuilder.builder<T>().apply(init).build()
}
